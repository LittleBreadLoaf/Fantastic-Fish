...
public class EntityMantaRay extends EntityWaterMob
{
	...
	public EntityMantaRay(World par1)
	{
		this(par1, rand.nextInt(5));
	}
	

	public EntityMantaRay(World par1World, int par3)
	{
		super(par1World);
		...
		if(FMLCommonHandler.instance().getEffectiveSide().isServer()){
			if(worldObj.getEntityByID(getBobberID()) instanceof EntityFishHook){
				...
				if(efh.field_146043_c != this){
					((EntityMantaRay)efh.field_146043_c).setBobberID(-1);
					efh.field_146043_c = this;
				}
			}
	...
	protected void updateAITasks()
	{
		...
		if(this.currentSwimTarget != null)
			Ycoord = currentSwimTarget.yCoord;
	...
	private void findRandomTarget(double X, double Y, double Z, boolean force) 
	{
		...
		xMovement = xMovement + ((int)Math.sin(slowTick));
		zMovement = zMovement + ((int)Math.cos(slowTick));
