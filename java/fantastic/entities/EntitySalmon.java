...
public class EntitySalmon extends EntityWaterMob
{
	...
	{
		this(par1, rand.nextInt(5), rand.nextInt(2));
	}

	public EntitySalmon(World par1World, int par3, int par4)
	{
		super(par1World);
		this.setPreRenderSize(par3);
		this.setTexture(par4);
		...
		if(FMLCommonHandler.instance().getEffectiveSide().isServer()){
			if(worldObj.getEntityByID(getBobberID()) instanceof EntityFishHook){
				...
				if(efh.field_146043_c != this){
					((EntitySalmon)efh.field_146043_c).setBobberID(-1);
					efh.field_146043_c = this;
				}
			}
	 ...
	 public boolean interact(EntityPlayer par1EntityPlayer)
	    {
		 	...
		 	if(item != null && item.getItem() == FantasticItems.fishingNet)
		 	{
		 		...
		 		droppedFish.setItemDamage(11 + this.getTexture());
