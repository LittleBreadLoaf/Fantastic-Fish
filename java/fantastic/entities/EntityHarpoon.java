...
public class EntityHarpoon extends Entity implements IProjectile
{
	private int xTile = -1;
	private int yTile = -1;
	private int zTile = -1;
	...
	public EntityPlayer angler = null;

	public EntityHarpoon(World var1)
	{
		super(var1);
	...
	public EntityHarpoon(World var1, double var2, double var4, double var6)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
		this.setSize(0.5F, 0.5F);
	...
	public EntityHarpoon(World var1, EntityLiving var2, EntityLiving var3, float var4, float var5)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
	...
	public EntityHarpoon(World var1, EntityLivingBase var2, float var3)
	{
		super(var1);
		this.renderDistanceWeight = 10.0D;
	...
	public void onUpdate()
	{
		...
		} else
		{
			...
			if (var4 != null)
			{
				if (var4.entityHit != null)
				{
					...
					if (var4.entityHit instanceof EntityLiving && this.angler != null)
					{
						this.ridingEntity = var4.entityHit;
						((EntityLiving)var4.entityHit).riddenByEntity = this;
		...
		if(this.angler != null)
		{
			if(this.getDistanceSqToEntity(this.angler) > 256)
			{
				double moveX = (this.posX - this.angler.posX);
				double moveY = (this.posY - this.angler.posY);
				double moveZ = (this.posZ - this.angler.posZ);
				double planeXZ = Math.sqrt(moveX*moveX + moveZ*moveZ);
				double angle = Math.atan2(moveZ, moveX);
				double angle2 = Math.atan2(moveY, planeXZ);
				
				if(angle2 > 8*Math.PI/15 || angle2 < 7*Math.PI/15)
				{
					moveX = -0.3 * (Math.cos(angle));
					moveZ = -0.3 * (Math.sin(angle));
				}
				else
				{
					moveX = 0;
				...
				moveY = -0.3 * (Math.sin(angle2));
				this.addVelocity(moveX, moveY, moveZ);
				if(this.ridingEntity != null)
					this.ridingEntity.addVelocity(moveX, moveY, moveZ);
			}
		}
	}
