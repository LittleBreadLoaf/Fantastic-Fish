...
public class EntityBasicFish extends EntityWaterMob
{
	...
	public EntityBasicFish(World par1)
	{
		this(par1, rand.nextInt(5), rand.nextInt(5));
	}

	public EntityBasicFish(World par1World, int par2, int par3)
	{
		super(par1World);
		this.setTexture(par2);
		this.setPreRenderSize(par3);
	...
	public void setBobberID(int par1)
	{
		if(FMLCommonHandler.instance().getEffectiveSide().isServer()){
			if(worldObj.getEntityByID(getBobberID()) instanceof EntityFishHook){
				EntityFishHook efh = (EntityFishHook) worldObj.getEntityByID(getBobberID());
				if(efh.bobber != this){
					((EntityBasicFish)efh.bobber).setBobberID(-1);
					efh.bobber = this;
				}
			}
	...
	{
		super.updateAITasks();
		if(getBobber() != null && getBobber().angler != null && (getBobber().angler.fishEntity == null)){
			setBobberID(-1);
		}
		...
		if(this.isInWater())
		{
			...
			setIsOutOfWater(1);
			if(getBobber() == null || getBobber().angler == null || getBobber().isDead || getBobber().angler.isDead)
			{
				setBobberID(-1);
			...
			{
				if(((EntityFishHook)playerThatsFishing.fishEntity).bobber == null)
				{
					((EntityFishHook)playerThatsFishing.fishEntity).bobber = this;
					setBobberID(playerThatsFishing.fishEntity.entityId);
					((EntityFishHook)playerThatsFishing.fishEntity).angler.fishEntity = getBobber();
				}
			}
			...
			if(getBobber() != null)
			{
				playerThatsFishing = getBobber().angler;
			}
	...
	private void approachTarget(double X, double Y, double Z, int speed) 
	{
		Vec3 vec3 = Vec3.createVectorHelper(this.currentSwimTarget.xCoord + 0.5D - X, this.currentSwimTarget.yCoord + 0.1D - Y, this.currentSwimTarget.zCoord + 0.5D - Z).normalize();
			...
			if(getBobber() != null)
			{
				...
				getBobber().setVelocity(this.motionX, this.motionY, this.motionZ);
			}
	...
	private void findRandomTarget(double X, double Y, double Z, boolean force) 
	{
		if(isInWater()){
			movementskip--;
			if(!force && getBobber() != null && getBobber().bobber == this && getBobber().angler != null){
				setEscape(getBobber().angler, this.posX, this.posY, this.posZ, true);
				return;
			}
			if(movementskip <= 0){
	...
	private void lookForBobber(double X, double Y, double Z) 
	{
		...
		for (int l = 0; l < list.size(); ++l)
		{
			...
			if(entity1 instanceof EntityFishHook)
			{
				if(ignoreTimer<=0)
				{
					if(((EntityFishHook)entity1).bobber == null)
					{
						((EntityFishHook)entity1).bobber = this;
						setBobberID(entity1.entityId);
						((EntityFishHook)entity1).angler.fishEntity = getBobber();
					}
				}
				else
				{
					...
					playerThatsFishing = ((EntityFishHook)entity1).angler;
				}
	...
	protected void collideWithEntity(Entity par1Entity)
	{
		if(par1Entity instanceof EntityFishHook)
		{
			...
			if(efh.bobber == null){
				setBobberID(efh.entityId);
				getBobber().bobber = this;
				efh.angler.fishEntity = getBobber();
				setEscape(efh.angler, this.posX, this.posY, this.posZ, true);
			}
		}
	 ...
	 public boolean interact(EntityPlayer par1EntityPlayer)
	    {
		 	...
		 	if(item != null && item.itemID == FantasticItems.fishingNet.itemID)
		 	{
		 		...
		 		droppedFish.setItemDamage(this.getTexture());
