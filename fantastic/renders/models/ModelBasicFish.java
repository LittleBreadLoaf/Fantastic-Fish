...
public class ModelBasicFish extends ModelBase
{
  ...
  float flopSpeed = 0.06662F;
  float flopAmount = 0.9F;
  float flopVelocity = 0.1F;
  int extraSpeed = 1;
  float sizeBoost = 1;
  ...
  public void setRotationAngles(float f, float f1, float f2, float f3, float f4, float f5, Entity entity)
  {
    ...
    flopVelocity = (float) Math.sqrt((entity.motionX*entity.motionX) + (entity.motionZ*entity.motionZ));
    if(entity instanceof EntityBasicFish)
    {
    	EntityBasicFish Fish = (EntityBasicFish)entity;
    	extraSpeed = Fish.extraSpeed;
    	sizeBoost = (float)(Fish.getRenderSize() * 2);
    
    
    
    
    if(Fish.getIsOutOfWater() == 1)
    {
    	flopSpeed = 0.11662F;
    	flopAmount = 0.08F;
    }
    ...
    {
    	flopSpeed = (float)(1.04F / sizeBoost);
    	flopAmount = (float)(0.3F / sizeBoost);
    }
    ...
    this.bodysection1.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
    this.bodysection2.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
    this.bodysection3.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
    this.bodysection4.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
    this.bodysection5.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
    this.bodysection6.rotateAngleY = flopAmount*extraSpeed*(MathHelper.cos(f2 *extraSpeed* flopSpeed) );
