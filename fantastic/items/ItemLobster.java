...
public class ItemLobster extends Item
{
	...
	{
		return Names.Lobster_UnlocalizedName[itemstack.getItemDamage()];
	}
    ...
    public boolean onItemUse(ItemStack par1ItemStack, EntityPlayer par2EntityPlayer, World par3World, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        else
        {
            ...
            if (par7 == 1 && Block.blocksList[i1] != null && Block.blocksList[i1].getRenderType() == 11)
            {
            	...
            	EntityLobster Lobster = new EntityLobster(par3World, this.getDamage(par1ItemStack));
            	Lobster.setLocationAndAngles((double)par4 + 0.5D, (double)par5 + d0, (double)par6 + 0.5D, 0, 0);
    			par3World.spawnEntityInWorld(Lobster);
    			Lobster.setHasNotSpawned(false);
    			
                if (Lobster != null)
                {
                    if (par1ItemStack.hasDisplayName())
                    {
                        Lobster.setCustomNameTag(par1ItemStack.getDisplayName());
                    }
            ...
            else
            {
                if (movingobjectposition.typeOfHit == EnumMovingObjectType.TILE)
                {
                    ...
                    if (par2World.getBlockMaterial(i, j, k) == Material.water)
                    {
                        ...
                        EntityLobster Lobster = new EntityLobster(par2World, this.getDamage(par1ItemStack));
            			Lobster.setLocationAndAngles((double)i, (double)j, (double)k, 0, 0);
            			par2World.spawnEntityInWorld(Lobster);
            			Lobster.setHasNotSpawned(false);
                        if (Lobster != null)
                        {
                            if (par1ItemStack.hasDisplayName())
